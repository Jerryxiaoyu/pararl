...
__all__ = ['KinovaJointControl']


class KinovaJointControl(gym.Env, metaclass=abc.ABCMeta,):
    ...
    def _get_obs(self):
        if self._isImageObservation:
            ...
            #kinovaState = self.kinova.GetMotorAngles()
            self._observation = \
                np.concatenate((self.kinova.GetMotorAngles()[:7],
                                self.kinova.GetMotorVelocities()[:7]
                                )).flatten()
        return self._observation
    def norm2realAction(self, action_norm):
        """
        [-1, 1] map to the real scale of joint velocity
        :return:
        """
        if self._isVelocity:
            action = action_norm * self.kinova.max_velocity
        else:
            action = action_norm * self.kinova.maxForce
    ...
    def step(self, action):
        ...
        """
        action mode : only joint velocity control
        :param action:
        :return:
        """
        finger_angle = 0
        action = self.norm2realAction(action)
        realAction = np.concatenate((action, [finger_angle, finger_angle, finger_angle] ))
    ...
    def step2(self, action):
        ...
        for i in range(self._actionRepeat):
            if self._isVelocity:
                self.kinova.ApplyAction_Velocity(action)
            else:
                self.kinova.ApplyAction_Torque(action)
        ...
        reward = self._reward(self._observation, action) + reward_terminal
